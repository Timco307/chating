...
createRoomBtn.addEventListener("click", async () => {
  ...
  if (!roomId) {
    roomId = Math.random().toString(36).substring(2, 8); // Generate random code if not provided
  } else {
    const roomDoc = await getDoc(doc(db, "rooms", roomId));
    if (roomDoc.exists()) return alert("Room code already exists. Choose another one.");
  }
